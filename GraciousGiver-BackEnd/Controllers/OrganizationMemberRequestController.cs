...
namespace GraciousGiver_BackEnd.Controllers
{
    ...
    [ApiController]
    public class OrganizationMemberRequestController : ControllerBase
    {
        private readonly GraciousDbContext _context;

        public OrganizationMemberRequestController(GraciousDbContext context)
        {
            _context = context;
        }
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutOrganizationMemberRequest(int id, OrganizationMemberRequest om)
        {
            if (id != om.OrganizationMemberRequestId)
            {
                return BadRequest();
            }
            if (ModelState.IsValid)
            {
                ...
                {
                    if (!OrganizationMemberRequestExists(id))
                    {
                        return NotFound();
                    }
                    else
        ...
        private bool OrganizationMemberRequestExists(int id)
        {
